...
$query =mysqli_query( $con,"SELECT NIP_pengaju,jenis_cuti,alasan_cuti,lama_cuti,tgl_mulai,tgl_selesai,alamat,provinsi,no_tlp,sp_kabid,sp_alasan FROM cuti_tahunan  WHERE  id_pengajuan='$id_pengajuan'  ");
$data= mysqli_fetch_array($query);

$NIP=$data['NIP_pengaju'];

$query2=mysqli_query($con, "SELECT data_pns.nama,data_pns.jabatan_eselon,anggota_bidang.kode_bidang,data_pns.masa_kerja,data_pns.jatah_tahunan FROM data_pns inner join anggota_bidang on anggota_bidang.NIP=data_pns.NIP  WHERE  data_pns.NIP='$NIP'  ");
...
//CATATAN CUTI
echo "<div style='margin-left:20px;'><h4><b>V. CATATAN CUTI</b></h4></div>";
echo "<table style='margin-left:40px;margin-right:20px;'>";
echo "<tr>";
echo "<td style='width:400px;'>Sisa Cuti Tahun ini :&nbsp;<b>".$data['lama_cuti']."&nbsp;</b>hari kerja.</td>";
echo "</tr>";
echo "</table>";

//ALAMAT SELAMA MENJALANKAN CUTI
echo "<div style='margin-left:20px;'><h4><b>VI. ALAMAT SELAMA MENJALANKAN CUTI</b></h4></div>";
